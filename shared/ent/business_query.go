...
func (bq *BusinessQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Business, init func(*Business), assign func(*Business, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(business.UsersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.business_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "business_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "business_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
